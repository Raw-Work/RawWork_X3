...
#define FW_IMAGE_LENS "synaptics/startup_fw_lens.img"
#define FW_IMAGE_OFILM "synaptics/startup_fw_ofilm.img"
#define FW_IMAGE_LENS_GFF "synaptics/startup_fw_lens_gff.img"
...
unsigned int customer_configuration_id;
...
static int fwu_read_f34_queries(void)
{
	...
	customer_configuration_id = extract_uint_be(config_id);
...
extern void synaptics_rmi4_reset(void);
...
static int fwu_start_reflash(void)
{
	...
	if (fwu->ext_data_source) {
		...
		if(!strcmp(fwu->product_id, "LL96_00"))
			strncpy(fwu->image_name, FW_IMAGE_LENS, MAX_IMAGE_NAME_LEN);
		else if(!strcmp(fwu->product_id, "LL96_01"))
			strncpy(fwu->image_name, FW_IMAGE_OFILM, MAX_IMAGE_NAME_LEN);
		else if(!strcmp(fwu->product_id, "S1_L02"))
			strncpy(fwu->image_name, FW_IMAGE_LENS_GFF, MAX_IMAGE_NAME_LEN);
		else{
			dev_err(&fwu->rmi4_data->i2c_client->dev,
...
exit:
	...
	retval = fwu_scan_pdt();
	if (retval < 0)
		printk("%s: fwu_scan_pdt fail !\n",__FUNCTION__);
#endif
	/* Get device config ID after flash FW*/
	retval = fwu->fn_ptr->read(fwu->rmi4_data,
				fwu->f34_fd.ctrl_base_addr,
				config_id,
				sizeof(config_id));
