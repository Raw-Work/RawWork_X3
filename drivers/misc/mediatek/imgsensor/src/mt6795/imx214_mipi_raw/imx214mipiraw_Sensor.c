...
extern kal_uint8 IMX214_Read_module_id(void);
...
{{ 4208, 3120,	  0,	0, 4208, 3120, 2104,  1560, 0000, 0000, 2104,  1560,	  0,	0, 2104, 1560}, // Preview 
 ...
 //{ 4208, 3120,	  0,  0, 4208, 3120, 1280,  720,  0000, 0000,  1280,  720,	  0,	0, 1280,  720}, //hight speed video 720p
 { 4208, 3120,	  0,    0, 4208, 3120, 2104,  1560, 0000,  376, 2104,  1184,	  0,	0, 2104, 1184}, //hight speed video 1080p
...
/*static void slim_video_setting() //for VT
{
	LOG_INF("E\n");
	preview_setting();
}*/
...
static uint32_t RG_Ratio_typical_Sharp=0x0102;
static uint32_t BG_Ratio_typical_Sharp=0x012D;

static uint32_t RG_Ratio_typical_Ofilm=0x0102;
static uint32_t BG_Ratio_typical_Ofilm=0x012D;
...
// return 0
int imx214_update_awb_gain()//uint32_t R_gain, uint32_t G_gain, uint32_t B_gain)
{
    
	LOG_INF("imx214_otp R_GAIN=0x%x G_GAIN=0x%x, B_GAIN=0x%x\n", R_GAIN, G_GAIN, B_GAIN);
...
int imx214_update_otp_wb(void)
{
	...
	if(IMX214_Read_module_id() == 0x01)
	{
        LOG_INF(" imx214_otp Sharp module awb\n");
        RG_Ratio_typical = RG_Ratio_typical_Sharp;
        BG_Ratio_typical = RG_Ratio_typical_Sharp;

		iReadData(0x01,4,awb_otp_data);//sharp
		
		rg_ratio = 512*awb_otp_data[0]/awb_otp_data[1];
		bg_ratio = 512*awb_otp_data[3]/awb_otp_data[1];

	}
	else if(IMX214_Read_module_id() == 0x07)
	{
        LOG_INF(" imx214_otp ofilm module awb\n");
        RG_Ratio_typical = RG_Ratio_typical_Ofilm;
        BG_Ratio_typical = BG_Ratio_typical_Ofilm;
		
		iReadData(0x09,4,awb_otp_data);//o-film

		rg_ratio = awb_otp_data[0] << 8 | awb_otp_data[1];
		bg_ratio = awb_otp_data[2] << 8 | awb_otp_data[3];
	}
	else
	{
        ...
        //calculate gain
        //0x400 = 1x gain
        LOG_INF(" imx214_otp current_otp.rg_ratio = 0x%x\n",rg_ratio);
        LOG_INF(" imx214_otp current_otp.bg_ratio = 0x%x\n",bg_ratio);
        LOG_INF(" imx214_otp RG_Ratio_typical = 0x%x\n",RG_Ratio_typical);
        LOG_INF(" imx214_otp BG_Ratio_typical = 0x%x\n",BG_Ratio_typical);

	r_ratio = 512 * RG_Ratio_typical / rg_ratio;
	b_ratio = 512 * BG_Ratio_typical / bg_ratio;
